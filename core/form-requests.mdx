...
FormRequests can also derive `InertiaProps` to generate TypeScript types, enabling end-to-end type safety from your Rust backend to your React frontend.

### Generating TypeScript Types for FormRequests

Add `InertiaProps` derive alongside `#[form_request]`:
...
1. Define FormRequest with validation in Rust
2. Add `#[derive(InertiaProps)]` to the struct
3. Run `kit generate-types` to generate TypeScript
4. Use the generated type with `useForm<FormRequestType>`
