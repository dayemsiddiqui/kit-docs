...
```rust
use kit::{handler, inertia_response, request, Request, Response, InertiaProps, redirect};
...
// PUT /todos/:id
#[handler]
pub async fn update(req: Request, form: TodoRequest) -> Response {
    let id: i32 = req.param("id").unwrap().parse().unwrap();
...
routes! {
    ...
    get("/todos/{id}/edit", todos::edit).name("todos.edit"),
    put("/todos/{id}", todos::update).name("todos.update"),
    post("/todos/{id}/toggle", todos::toggle).name("todos.toggle"),
    delete("/todos/{id}", todos::destroy).name("todos.destroy"),
}
```
...
export default function TodoIndex({ todos }: TodoIndexProps) {
  const toggleTodo = (id: number) => {
    router.visit(controllers.todos.toggle({ id: String(id) }))
  }

  const deleteTodo = (id: number) => {
    if (confirm('Are you sure?')) {
      router.visit(controllers.todos.destroy({ id: String(id) }))
    }
  }
        ...
        <ul className="space-y-3">
          {todos.map((todo) => (
            ...
            >
              ...
              <Link
                href={controllers.todos.edit({ id: String(todo.id) })}
...
export default function TodoEdit({ todo }: TodoFormProps) {
  ...
  return (
    <div className="max-w-md mx-auto p-8">
      ...
      <Form action={controllers.todos.update({ id: String(todo.id) })} className="space-y-4">
        {({ processing }) => (
          <>
            <div>
              <label className="block text-sm font-medium mb-1">Title</label>
