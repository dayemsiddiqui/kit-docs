...
description: 'Working with database models using Queryable and QueryableMut traits'
icon: 'cube'
---
...
The `Queryable` trait provides read-only operations for querying data:

```rust
use kit::database::Queryable;
use crate::models::todos::Entity as Todos;
```

<Note>
Implementing `kit::database::Queryable` also enables [Route Model Binding](/core/routing#route-model-binding), allowing you to use `Model` types directly as handler parameters for automatic database lookup.
</Note>
...
## The QueryableMut Trait

The `QueryableMut` trait provides write operations for creating, updating, and deleting data:

```rust
use kit::database::QueryableMut;
...
use kit::database::{Queryable, QueryableMut};
...
|-------|--------|-------------|
| `Queryable` | `all()` | Get all records |
| `Queryable` | `find_by_pk(id)` | Find by primary key |
| `Queryable` | `find_or_fail(id)` | Find or error |
| `Queryable` | `first()` | Get first record |
| `Queryable` | `count_all()` | Count all records |
| `Queryable` | `exists_any()` | Check if any exist |
| `QueryableMut` | `insert_one(model)` | Insert a record |
| `QueryableMut` | `update_one(model)` | Update a record |
| `QueryableMut` | `save_one(model)` | Insert or update |
| `QueryableMut` | `delete_by_pk(id)` | Delete by primary key |
